...
func run(arguments []string) error {
	...
	case "config":
		settings, err := parseConfig(arguments[1:])
		if err != nil {
			return err
		}
		encoder := json.NewEncoder(os.Stdout)
...
func parseConfig(arguments []string) (config.Config, error) {
...
func serve(arguments []string) error {
	settings, err := parseConfig(arguments)
	if err != nil {
		return err
	}
	logger, err := observability.NewLogger(os.Stdout, settings.LogFormat)
	if err != nil {
		return err
	}
